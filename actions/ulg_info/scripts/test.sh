...
clean_actual_output() {
    ...
    find "$ACTUAL_OUTPUT_DIR" -type f -name '.*' -delete
}
...
main() {
    # Test 1
    echo "Running Test 1: Basic ulog info test"
    clean_actual_output
    run_docker_test ""
    compare_outputs "test.ulg.md" "test.ulg.md"

    # Test 2
    echo "Running Test 2: Test for .test.ulg.md with ROBOTO_PARAM_HIDDEN=True"
    clean_actual_output
