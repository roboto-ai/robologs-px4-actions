...
echo "Pushing px4_flight_review:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(px4_flight_review:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push px4_flight_review:latest to Roboto's private registry"
    exit 1
fi

echo "Creating px4_flight_review action"
create_args=(
